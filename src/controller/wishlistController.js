...
const addToWishlist = async function (req, res) {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .send({ status: false, message: "invalid userId and token " });
    }
    ...
    if (!userWishlist) {
      ...
      return res
        .status(201)
        .send({ status: true, message: "Added to wishlist", wishlist });
    } else {
...
const getWishlist = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Success", wishlist: userWishlist });
  } catch (error) {
