...
const signUp = async (req, res) => {
  try {
    ...
    return res.cookie("x-api-key", token).status(201).send({ status: true, message: "Success", data: savedData ,token:token});
  } catch (err) {
...
const loginUser = async function (req, res) {
  try {
    ...
    res.status(200).send({ success: true, user: userInfo, response ,token:token});
